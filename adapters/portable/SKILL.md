...
- `grok` through Cursor ask is the default independent review route when it is independent of the current worker and coordinator. `antigravity` ask is the alternate reviewer; native `grok-build` (ask and work) is off the default path after repeated timeouts and cancellations; Antigravity work is an alternate implementation route. Do not call a same-family review independent.
...
Use a small finite budget by default: up to six external calls including failures, two concurrent workers in separate workspaces, two implementation attempts per package, and 900 seconds per call (1200 seconds for Cursor Grok reviews). State a larger finite budget before dispatch for a larger task. Preserve partial work and return real failures to its owner for repair.
